...
#辞書作成
    ...
    for player in match_data.get('players', []):
        p_id = player.get('trackable_object') #選手ID
        t_id = player.get('team_id') #チームID
        
        if p_id is not None:
            role_map[p_id] = 'home' if t_id == home_id else 'away' #ホームかアウェイか
...
def create_animation(tracking_data, role_map):
    ...
    # loc='lower center' で中央下に、bbox_to_anchor で位置を微調整
    ...
    def update(i):
        ...
        for person in frame_data.get('data', []):
            ...
            if x is not None and y is not None: #座標データが存在する場合のみ以下の処理を行う
    ...
    #指定した関数を、一定の時間間隔で何度も繰り返し実行してくれるタイマー付きの描画システム
    ani.save("tracking_101887.mp4", writer='ffmpeg')
    print("Animation saved as tracking_101887.mp4")
